...
{
    public string $table = Driver::TABLE_NAME;
    ...
    private function getDriverQuery(): Builder
    {
        return $this->getQuery()
            ->select(
                'drivers.id',
                'drivers.institute_id',
                'drivers.branch_id',
                'drivers.name',
                'drivers.phone_number',
                'drivers.license_number',
                'drivers.assigned_bus_id',
                'drivers.status'
            );
    }
    ...
    protected function filterSearchQuery(Builder|EloquentBuilder $query, string $searchedText): Builder
    {
        ...
        return $query->where('drivers.name', 'LIKE', $searchable)
            ->orWhere('drivers.phone_number', 'LIKE', $searchable)
            ->orWhere('drivers.status', 'LIKE', $searchable);
    }
    ...
    protected function getExceptionMessages(): array
    {
        ...
        $userExceptionMessages = [
            static::MESSAGE_ITEM_DOES_NOT_EXIST_MESSAGE => 'Driver does not exist.',
            static::MESSAGE_ITEM_COULD_NOT_BE_DELETED => 'Driver could not be deleted.',
        ];
