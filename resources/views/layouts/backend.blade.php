...
<head>
    ...
    <style>
        ...
        .modal-content {
            border: 1px solid var(--kt-modal-border-color) !important;
        }
        ...
        #User-table_wrapper .dt-length label {
...
<body id="kt_app_body" data-kt-app-layout="light-sidebar" data-kt-app-sidebar-enabled="true"
                        ...
                        <div class="flex-shrink-0 app-navbar">
                            ...
                            <div class="app-navbar-item ms-1 ms-lg-3">
                                @if (Auth::User()->user_type === 'System Admin' ||
                                        Auth::User()->user_type === 'SAASAdmin' ||
                                        Auth::User()->user_type === 'Super Admin')
                                    <select class="select_class form-select form-select-sm form-select-solid"
                                        data-control="select2" onchange="confirmChangeBranch(this);"
                                        style="margin-top: 10px;">
                                        @foreach (get_table('branches') as $branch)
                                            <option value="{{ $branch->id }}"
                                                {{ $branch->id == get_branch_id() ? 'selected' : '' }}>
                                                {{ _lang('Branch') }} ({{ $branch->name }})
                                            </option>
                                        @endforeach
                                    </select>
                                @endif
                                ...
                                @if (Auth::User()->user_type === 'System Admin' ||
                                        Auth::User()->user_type === 'Super Admin' ||
                                        Auth::User()->user_type === 'Accountant' ||
                                        Auth::User()->user_type === 'Librarian')
                                    <select class="select_class form-select form-select-sm form-select-solid"
                                        data-control="select2" onchange="confirmChangeSession(this);"
                                        style="margin-top: 10px;">
                                        @foreach (get_table('academic_years') as $session)
                                            <option value="{{ $session->id }}"
                                                {{ $session->id == get_option('academic_year') ? 'selected' : '' }}>
                                                {{ _lang('Session') }} ({{ $session->year }})
                                            </option>
                                        @endforeach
                                    </select>
                                @endif
                            ...
                            <div class="app-navbar-item ms-1 ms-lg-3" id="kt_header_user_menu_toggle">
                                ...
                                <div class="py-4 menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg menu-state-color fw-semibold fs-6 w-275px"
                                    ...
                                    <div class="px-5 menu-item">
                                        <a href="{{ url('profile/my-profile') }}" class="px-5 menu-link">My
                                            Profile</a>
                                    </div>
                                    <!--end::Menu item-->
                                                ...
                                                <span
                                                    ...
                                                    @if (Session::get('locale') == 'bn')
                                                        ...
                                                        <img class="w-15px h-15px rounded-1 ms-2"
                                                            src="{{ asset('assets') }}/media/flags/bangladesh.svg"
                                                            alt="" />
                                                    @else
                                                        English
                                                        <img class="w-15px h-15px rounded-1 ms-2"
                ...
                <div id="kt_app_sidebar" class="app-sidebar flex-column" data-kt-drawer="true"
                    ...
                    <div class="overflow-hidden app-sidebar-menu flex-column-fluid">
                        ...
                        <div id="kt_app_sidebar_menu_wrapper" class="my-5 app-sidebar-wrapper hover-scroll-overlay-y"
                            ...
                            <!--begin::Menu-->
                            <div class="px-3 menu menu-column menu-rounded menu-sub-indention"
                                ...
                                @if (Auth::User()->user_type === 'Teacher')
                                    @include('layouts.menus.teacher_sidebar')
                                @endif

                                @if (Auth::User()->user_type === 'Student')
                                    @include('layouts.menus.student_sidebar')
                                @endif
                            ...
                            <div id="kt_app_toolbar_container"
                                ...
                                <div class="flex-wrap page-title d-flex flex-column justify-content-center me-3">
                                    ...
                                    <h1
                                        ...
                                        @yield('page_title_breadcrumb', 'Dashboard')
                                    </h1>
                                    <!--end::Title-->
                                    <!--begin::Breadcrumb-->
                                    ...
                                    <ul class="pt-1 my-0 breadcrumb breadcrumb-separatorless fw-semibold fs-7">
                                        ...
                                        @foreach ($segments as $segment)
                                            ...
                                            @php $urlSegments .= '/'.$segment; @endphp
                                            ...
                                            <li class="breadcrumb-item text-muted">
                                                <a
                                                    href="{{ url($urlSegments) }}">{{ ucwords(str_replace('_', ' ', $segment)) }}</a>
                                            </li>
                                            <!--end::Item-->
                                        @endforeach

                                        {{-- <!-- Optionally add the current page name -->
                                        @if (!empty($segments))
                                            <li class="breadcrumb-item">
                                                <span class="bg-gray-400 bullet w-5px h-2px"></span>
                                            </li>
                                            <li class="breadcrumb-item text-muted">
                                                {{ ucwords(str_replace('_', ' ', end($segments))) }}</li>
                                        @endif --}}
                                    </ul>
                                ...
                                <!--end::Page title-->
                                <div>
                                    @if (view()->hasSection('create-button'))
                                        @yield('create-button')
                                    @endif

                                    @if (view()->hasSection('create-button-2'))
                                        @yield('create-button-2')
                                    @endif
                                </div>
                            </div>
    ...
    <script>
        ...
        function confirmChangeSession(elem) {
            if ($(elem).val() == "") {
                return;
            }
            if (confirm("Are you sure you want to change the Session?")) {
                changeSession(elem);
            }
        }

        function changeSession(elem) {
            window.location = "{{ url('administration-change-session') }}/" + $(elem).val();
        }

        function confirmChangeBranch(elem) {
            if ($(elem).val() == "") {
                return;
            }
        ...
        function changeBranch(elem) {
            window.location = "{{ url('administration-change-branch') }}/" + $(elem).val();
        }
    </script>
    <!--end::Javascript-->
</body>
